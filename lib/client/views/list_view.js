...
ListView.prototype.loadModel = function(model) {
  ...
  for(i = 0 ; i < this.model.length ; i++) {
    ...
    this.setupItemView(itemModel, i)
  }
  ...
  this.model.on("insert", function(newDoc, index) {
    var doInsert = function() {
      that.setupItemView(newDoc, index)
      that.view.set(that.model.collectionName + "Count", that.model.length)
    }
...
ListView.prototype.setupItemView = function(itemModel, index) {
  //append a blank document, we'll set it in SingleView
  new this.SingleView(itemModel, this.viewCollection, { append: index, parent: this })
}
